// src/pages/POSPage.jsx – complete real-world POS cashier workflow
...
export default function POSPage() {
  ...
  const { data: customers = [] } = useQuery({
    ...
    queryFn:  async () => { const { data } = await api.get('/customers'); return data.data; },
    enabled:  isOnline,
    staleTime:1000 * 60 * 5,
  });
...
const filtered = allProducts.filter((p) => {
  ...
  const { mutate: checkout, isPending: isCheckingOut } = useMutation({
    mutationFn: async (payload) => {
      ...
      return { offlineId, isOffline: true, invoiceNumber: `OFFLINE-${Date.now()}`, items: cart.items.map(i => ({
        productName: i.product.name, quantity: i.quantity, unitPrice: i.unitPrice,
        discount: i.discount, lineTotal: i.unitPrice * i.quantity - (i.discount||0)
      })), grandTotal: cart.grandTotal(), paymentMethod: cart.paymentMethod, date: new Date() };
  ...
  toast.success(`+ ${product.name || 'Product'}`, {
    ...
    style: { fontSize: '13px' }
                ...
                >
                  ...
                  <p className="font-semibold text-sm text-gray-900 leading-tight mb-1 pr-6">
                    {product.name}
                  </p>
...
function BarcodeScanner({ onResult, onClose }) {
  ...
  useEffect(() => {
    ...
    import('html5-qrcode').then(({ Html5QrcodeScanner }) => {
      scanner = new Html5QrcodeScanner('qr-reader', { fps: 10, qrbox: { width: 260, height: 130 } }, false);
      scanner.render(text => { scanner.clear().catch(() => {}); onResult(text); }, () => {});
    });
  ...
  return (
    <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl p-4 w-full max-w-sm">
        ...
        <p className="text-xs text-center text-gray-400 mt-3">Point your camera at a barcode</p>
      </div>
    </div>
  );
}
