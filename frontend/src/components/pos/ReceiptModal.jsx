// src/components/pos/ReceiptModal.jsx – thermal receipt + WhatsApp share v4
...
export default function ReceiptModal({ invoice, onClose, cashReceived, changeAmount }) {
  ...
  // ✅ FIX 1 – use contentRef instead of content
  ...
  const handleShare = async () => {
    ...
    if (navigator.share) {
      try { await navigator.share({ title: `Receipt ${invoice.invoiceNumber}`, text }); } catch (_) {}
      ...
      >
        ...
        >
                ...
                {cashReceived > 0 && invoice.paymentMethod === 'Cash' && (
                  <>
                    ...
                    <div className="flex justify-between text-sm font-bold text-green-600">
                      <span>Change</span>
                      <span>${parseFloat(changeAmount || 0).toFixed(2)}</span>
                    </div>
                  </>
                )}
          ...
          <div className="flex gap-2 px-4 pb-5 pt-3 border-t border-gray-100 print:hidden">
            <button
              onClick={handlePrint}
              className="flex-1 py-3 rounded-2xl font-bold flex items-center justify-center gap-1.5 text-white text-sm"
            ...
            <button
              ...
              className="flex-1 py-3 rounded-2xl font-bold flex items-center justify-center gap-1.5 text-white text-sm bg-green-600"
            >
              <MessageCircle size={16} /> WhatsApp
            </button>
            <button
              onClick={handleShare}
              className="py-3 px-4 rounded-2xl font-bold flex items-center justify-center text-sm"
